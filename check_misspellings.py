...
def check_python_file(filename, corpus):
    return check_tokenstream(pyfile_token_stream(filename), corpus)
...
def check_text_file(filename, corpus):
    return check_tokenstream(textfile_token_stream(filename), corpus)
...
def token_in_corpus(token, corpus):
    ...
    if "-" in token or "_" in token:
        if _case_insensitive_str_in_corpus(
            ...
            return True
        if all(
            (_case_insensitive_str_in_corpus(x, corpus) or should_skip_token(x))
...
def check_tokenstream(tokens, corpus):
    ...
    for token, line, lineno, pos in tokens:
        ...
        matches = difflib.get_close_matches(
            token, corpus, cutoff=get_cutoff_for_token(token)
        )
        if matches:
...
def main():
    ...
    if args.use_system_dict:
        if sys.platform.startswith("linux") and os.path.exists("/usr/share/dict/words"):
            args.corpus.append("/usr/share/dict/words")
        else:
            print("Warning: No found system dict for current platform.")
